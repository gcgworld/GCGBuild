...
## Copy everything but the filesystem to the dir for our future image.
rsync --verbose --exclude=/casper/filesystem.squashfs -a $INITIAL_MOUNT/ $BUILD_MOUNT

## Load the squashfs kernel module
modprobe --verbose squashfs

## Mount the squashfs file system.
mount --verbose -t squashfs -o loop $INITIAL_MOUNT/casper/filesystem.squashfs $FS_MOUNT/

## Copy the mounted squashfs file system to where we are going to edit the image.
cp --verbose -a $FS_MOUNT/* $EDIT_MOUNT

## Copy files we are going to need for networking into the image
...
## Once Chrooted script will pause until user exits editing context.
## Should probably account for system shutdown, or closing the terminal, the process dying..
## Just checks to see where it was at when it died.
